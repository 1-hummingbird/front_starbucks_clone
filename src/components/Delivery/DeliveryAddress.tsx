...
function DeliveryAddress() {
  ...
  const handleSubmit = async (e) => {
    ...
    // API에 맞는 데이터 구조로 변환
    const apiData = {
      addressNickname: formData.addressalias, // 주소 별칭
      name: formData.name, // 받는 분 이름
      address: formData.address, // 기본 주소
      phone: formData.tel, // 연락처
      memo: formData.note, // 배송 메모
      isDefault: formData.isDefault, // 기본 배송지 여부
    };
    ...
    try {
      ...
      const response = await fetch("/api/v1/shipping/get-default", {
        ...
        body: JSON.stringify(apiData), // 서버에 전송할 데이터
      });
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="py-3">
            <input
              ...
              onChange={handleInputChange}
            />
            <label className="ml-2 text-black">기본 배송지로 저장합니다.</label>
          </div>
